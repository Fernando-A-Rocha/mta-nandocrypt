...
A `nando_decrypter` script file will be created, as well as `nando_decrypter_keys` which stores the necessary keys used in nando_decrypter to decrypt your files alongside the secret key stored in it.
...
Keep `nando_decrypter_keys` in the nandoCrypt resource because it is used by the ecrypter to generate the `nando_decrypter` script.
